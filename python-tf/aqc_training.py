...
def create_reader(filenames,n_subj=3331,n_samples=57848,batch_size=32,training_frac=90,validation_frac=2,testing_frac=8):
    """
    ...
    def _remove_subj(a,b):
        return a,{'qc':tf.expand_dims(b['qc'],axis=-1)}
...
def main():
    ...
    # convert to tensorflow estimator
    est_model = tf.keras.estimator.model_to_estimator(keras_model=model,model_dir='training')

    #print(est_model.input_names)
    ...
    def train_input_fn():
       training_ds,testing_ds,validation_ds = create_reader([params.data], n_subj=params.subjects, n_samples=params.size, batch_size=params.batch_size)
       return training_ds
