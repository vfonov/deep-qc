...
for(j in Sys.glob(file.path(inp,'log_*_*.json'))) {
    ...
    fold<-bind_rows(
        ...
        as.data.frame(rrr$testing_best_loss$summary)%>%mutate(optimize='loss'))
    cv<-bind_rows(cv,fold)

    fold_details <- bind_rows(
        as.data.frame(rrr$testing_final$details)   %>%mutate(optimize='final'),
        as.data.frame(rrr$testing_best_loss$details)%>%mutate(optimize='loss')) %>%
...
details<-details%>%mutate(optimize=factor(optimize, levels=c('final', 'loss') ))
cv<-cv %>% mutate(optimize=factor(optimize, levels=c('final', 'loss') ))

ccv<-cv %>% gather(`loss`, key='measure', value='score')
tccv<-ccv %>% group_by(measure, optimize) %>% 
  summarize( score=median(score), score_lab=signif(score,4))%>%ungroup()

# select the best loss
best_loss<-min( (tccv%>%filter(measure=='loss'))$score)

# highlite
tccv<-tccv %>% mutate( highlite=(score==best_loss)&(measure=='loss') )
...
for(s in levels(details$optimize)) {
   ...
   r<-roc(ss$labels, ss$preds, stratified=F, auc=T) # 
